...
const LABEL = "com.devmatrix.pixlet";
const UNIT = "dmx-pixlet.service";
const INSTALLER_PATH = fileURLToPath(import.meta.url);
const EXAMPLES_DIR = path.dirname(INSTALLER_PATH);
const BRIDGE_PATH = path.resolve(EXAMPLES_DIR, "pixlet-bridge", "bridge.mjs");
const DEFAULT_CONFIG_PATH = path.resolve(EXAMPLES_DIR, "pixlet-bridge", "bridge.config.json");
...
function usage() {
  return `Usage: node examples/install-pixlet-bridge.mjs [mode] [options]
...
Install options:
  --config PATH             Bridge JSON (BRIDGE_CONFIG; defaults to the package config)
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!CONFIG_FLAGS.has(flag)) throw new Error(`Unknown option: ${flag}`);
...
function validateConfig(env) {
  ...
  if (!path.isAbsolute(env.BRIDGE_CONFIG)) {
    throw new Error("BRIDGE_CONFIG must resolve to an absolute path.");
  }
  if (!fs.existsSync(env.BRIDGE_CONFIG) || !fs.statSync(env.BRIDGE_CONFIG).isFile()) {
    throw new Error(`Bridge config not found: ${env.BRIDGE_CONFIG}`);
  }
}

async function collectConfig(initialEnv) {
  const env = {
    BRIDGE_CONFIG: path.resolve(initialEnv.BRIDGE_CONFIG || DEFAULT_CONFIG_PATH),
    ...(initialEnv.DMX_TOKEN !== undefined ? { DMX_TOKEN: initialEnv.DMX_TOKEN } : {}),
  };
  if (env.DMX_TOKEN) {
    validateConfig(env);
    return env;
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error("Pass --token when input is not an interactive terminal.");
  }
...
function pathsForPlatform(platform = process.platform) {
  if (platform === "linux") {
    return {
      ...
      envFile: "/etc/devmatrix/pixlet.env",
      serviceFile: "/etc/systemd/system/dmx-pixlet.service",
    };
  }
  if (platform === "darwin") {
    const userHome = os.homedir();
    return {
      envDirectory: path.join(userHome, "Library", "Application Support", "devmatrix"),
      envFile: path.join(
        userHome,
        "Library",
        "Application Support",
      ...
      logDirectory: path.join(userHome, "Library", "Logs", "devmatrix"),
      stderrLog: path.join(userHome, "Library", "Logs", "devmatrix", "pixlet-error.log"),
      stdoutLog: path.join(userHome, "Library", "Logs", "devmatrix", "pixlet.log"),
      serviceFile: path.join(userHome, "Library", "LaunchAgents", `${LABEL}.plist`),
    };
  }
...
function orderedEnvironment(env) {
  return ["BRIDGE_CONFIG", "DMX_TOKEN"]
...
function renderSystemdUnit() {
  return `[Unit]
Description=Devmatrix Pixlet Bridge
After=network-online.target

[Service]
EnvironmentFile=/etc/devmatrix/pixlet.env
ExecStart=${quoteSystemdArgument(NODE_PATH)} ${quoteSystemdArgument(BRIDGE_PATH)}
Restart=always
RestartSec=5
...
function printInstallPlan(env, paths) {
  ...
  console.log(`Devmatrix Pixlet bridge install dry run — ${platformName}`);
  console.log(`Node: ${NODE_PATH}`);
  console.log(`Bridge script: ${BRIDGE_PATH}`);
  console.log(`Bridge config: ${env.BRIDGE_CONFIG}`);
...
function printUninstallPlan(options, paths) {
  const platformName = process.platform === "darwin" ? "macOS (launchd)" : "Linux (systemd)";
  console.log(`Devmatrix Pixlet bridge uninstall dry run — ${platformName}`);
  if (process.platform === "linux") {
...
async function main() {
  ...
  if (!fs.existsSync(BRIDGE_PATH)) throw new Error(`Bridge script not found: ${BRIDGE_PATH}`);
...
main().catch(error => {
  ...
  console.error(`install-pixlet-bridge: ${message}`);
  process.exitCode = 1;
});
