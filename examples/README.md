...
| Script | What it does |
|---|---|
| [`dmx-top.mjs`](dmx-top.mjs) | A live terminal panel for one DK-01 (zero dependencies, Node 20+): vitals, what the panel is showing, per-app fetch diagnostics from `/api/v1/apps/diag` (firmware 0.10.0+), an event log, and a command line — `pair`, `text`, `show`, `weather`, `flights <url>` / `flights prompt`, `bright`, raw `get`/`post`. Config via flags, `DMX_URL`/`DMX_TOKEN`, or `~/.dmx-top.json` (written by `pair`, mode `0600`). Start: `node examples/dmx-top.mjs --device http://dmx-xxxx.local`, then type `help`. |
